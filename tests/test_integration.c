...
static TuiWorkspace* make_dummy_ws(void) {
    ...
    ws->id = tui_next_id();
    strncpy(ws->name, "IntWS", sizeof(ws->name) - 1);
...
static TuiTab* make_dummy_tab(void) {
    ...
    tab->id = tui_next_id();
    strncpy(tab->name, "IntTab", sizeof(tab->name) - 1);
...
void test_integration_full_lifecycle(void) {
    ...
    TEST_ASSERT_EQUAL_INT(1, tab->window_count);
...
void test_integration_rapid_create_destroy(void) {
    ...
    TEST_ASSERT_EQUAL_INT(50, mgr->workspace_count);
...
void test_integration_tab_cycling(void) {
    ...
    TEST_ASSERT_EQUAL_INT(5, ws->tab_count);
    ...
    TEST_ASSERT_EQUAL_INT(4, ws->active_tab_index);
    ...
    TEST_ASSERT_EQUAL_INT(2, ws->active_tab_index); /* 1 -> 2 */

    tui_manager_process_keyboard(mgr, NCKEY_RIGHT, &ni);
    TEST_ASSERT_EQUAL_INT(3, ws->active_tab_index); /* 2 -> 3 */

    tui_manager_process_keyboard(mgr, NCKEY_RIGHT, &ni);
    TEST_ASSERT_EQUAL_INT(4, ws->active_tab_index); /* 3 -> 4 */

    tui_manager_process_keyboard(mgr, NCKEY_RIGHT, &ni);
    TEST_ASSERT_EQUAL_INT(0, ws->active_tab_index); /* 4 -> 0 (wrap) */

    /* LEFT arrow wraps backward */
    tui_manager_process_keyboard(mgr, NCKEY_LEFT, &ni);
    TEST_ASSERT_EQUAL_INT(4, ws->active_tab_index); /* 0 -> 4 (wrap) */
...
void test_integration_window_close_reopen(void) {
    ...
    TEST_ASSERT_EQUAL_INT(2, tab->window_count);
    ...
    TEST_ASSERT_EQUAL_INT(2, tab->window_count);
    TEST_ASSERT_EQUAL_INT(1, tab->active_window_index); /* newest window is active */
...
void test_integration_text_input_in_window(void) {
    ...
    TuiTextInput* ti = tui_text_input_create(win->plane, 0, 0, 40, spy_submit, NULL);
...
void test_integration_remove_workspace_with_windows(void) {
    ...
    TEST_ASSERT_EQUAL_INT(0, mgr->active_workspace_index); /* ws0 is active */
