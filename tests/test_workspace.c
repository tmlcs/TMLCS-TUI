...
static TuiManager* make_dummy_manager(void) {
    ...
    mgr->_nc = NULL;
    mgr->_stdplane = (struct ncplane*)0xCAFE;
    mgr->_workspace_count = 0;
    mgr->_workspace_capacity = 4;
    mgr->_workspaces = (TuiWorkspace**)calloc(4, sizeof(TuiWorkspace*));
...
void test_workspace_name_is_set(void) {
    ...
    TEST_ASSERT_NOT_NULL(ws);
    TEST_ASSERT_EQUAL_STRING("DevWS", ws->_name);
    tui_workspace_destroy(ws);
    free(mgr->_workspaces);
    free(mgr);
}
...
void test_workspace_set_active_tab(void) {
    ...
    TEST_ASSERT_EQUAL_INT(2, ws->_tab_count);
...
void test_workspace_set_active_tab_invalid_index(void) {
    ...
    TEST_ASSERT_TRUE(ws->_active_tab_index == 0);
