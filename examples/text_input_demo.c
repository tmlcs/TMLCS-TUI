...
static void render_console(TuiWindow* win) {
    ...
    ncplane_set_fg_rgb(win->plane, THEME_BG_TAB_ACTIVE);
    ncplane_putstr_yx(win->plane, 1, 1, "[ Type below and press Enter ]");
    if (g_input) {
...
int main(void) {
    ...
    win->user_data = "Console";
    win->render_cb = render_console;
    tui_tab_add_window(tab, win);
    ncplane_move_yx(win->plane, 2, 2);

    g_input = tui_text_input_create(win->plane, 3, 1, 58, on_submit, NULL);
    g_input->focused = true;
    win->text_input = g_input;
